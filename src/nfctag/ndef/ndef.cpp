...
typedef uint8_t(*composeRtdPtr) (const NDEFDataStr * ndef,
                                 NDEFRecordStr * ndefRecord, uint8_t * I2CMsg);
...
static bool NT3HReadHeaderNfc(uint8_t * endRecordsPtr, uint8_t * ndefHeader)
{
        *endRecordsPtr = 0;
        ...
        if (ret == true) {
                ...
                if ((NDEF_START_BYTE == nfcPageBuffer[0]) &&
                    (NTAG_ERASED != nfcPageBuffer[2])) {
...
int16_t firstRecord(UncompletePageStr * page, const NDEFDataStr * data,
                    RecordPosEnu rtdPosition)
{
        ...
        uint8_t recordLength =
            composeRtd[typeFunct] (data, &record,
                                   &nfcPageBuffer[sizeof(NDEFHeaderStr)]);
...
int16_t addRecord(UncompletePageStr * pageToUse, const NDEFDataStr * data,
                  RecordPosEnu rtdPosition)
{
        NDEFRecordStr record {
        };
        ...
        if (pageToUse->usedBytes + recordLength < NFC_PAGE_SIZE) {
                memcpy(&nfcPageBuffer[pageToUse->usedBytes], tmpBuffer,
                       recordLength);
                ...
                memcpy(nfcPageBuffer, &tmpBuffer[byteToCopy],
                       pageToUse->usedBytes);
...
static bool writeUserPayload(int16_t payloadPtr, const NDEFDataStr * data,
                             UncompletePageStr * addPage)
{
        ...
        if ((addedPayload >= data->rtdPayloadlength) &&
            ((payloadPtr + copyByte) < NFC_PAGE_SIZE)) {
        ...
        while (!endRecord) {
                ...
                if (addedPayload < data->rtdPayloadlength) {
                        ...
                        if ((data->rtdPayloadlength - addedPayload) <
                            NFC_PAGE_SIZE) {
                                memcpy(nfcPageBuffer,
                                       &data->rtdPayload[addedPayload],
                                       (data->rtdPayloadlength - addedPayload));
                                nfcPageBuffer[(data->rtdPayloadlength -
                                               addedPayload)] = NDEF_END_BYTE;
                        } else {
                                memcpy(nfcPageBuffer,
                                       &data->rtdPayload[addedPayload],
                                       NFC_PAGE_SIZE);
...
typedef int16_t(*addFunct_T) (UncompletePageStr * page,
                              const NDEFDataStr * data,
                              RecordPosEnu rtdPosition);
...
bool NT3HwriteRecord(const NDEFDataStr * data)
{
        ...
        if (data->ndefPosition != NDEFFirstPos) {
                ...
                addPage.page =
                    (recordLength + sizeof(NDEFHeaderStr) + 1) / NFC_PAGE_SIZE;
        ...
        int16_t payloadPtr =
            addFunct[data->ndefPosition] (&addPage, data, data->ndefPosition);
        if (payloadPtr == -1) {
